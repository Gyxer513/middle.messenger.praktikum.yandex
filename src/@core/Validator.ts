export default class Validator {
  private rules: { [key: string]: (value: string) => [boolean, string] } = {
    ...
    repeat_password: this.validatePassword,
  ...
  private validateName(value: string): [boolean, string] {
    ...
    if (!/^[A-ZА-Я][a-zа-яA-ZА-Я-]*$/.test(value)) {
      return [
        false,
        'Имя должно начинаться с заглавной буквы, содержать только буквы латиницы или кириллицы и может включать дефис.'
      ];
    }
  ...
  private validateDisplayName(value: string): [boolean, string] {
    ...
    if (!/^[a-zA-Z0-9]+$/.test(value)) {
      return [
        false,
        'Ник может содержать только буквы латинского алфавита и цифры.'
      ];
    }
